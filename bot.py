...
"""
游刃有余双冷方案 · 最终自动补全版
启动后自动补全近期历史，无需 Volume 即可生成滚动10条三期计划。
"""
...
def get_recommendation(win, state):
    ...
    if morph["triggered"]:
        mapping = {"连续出单":("大单","小单"),"连续出双":("大双","小双"),"大双小单交替":("大双","小单"),
                   "大单小双交替":("大单","小双"),"连续出大":("大双","大单"),"连续出小":("小双","小单"),
                   "单双跳":("大单","小双"),"大小跳":("大单","小双")}
        a_rec,b_rec = mapping.get(morph["type"],("大双","小双"))
        return {"a":a_rec,"b":b_rec,"aPeriod":1,"bPeriod":1,"cnt":cnt,"isMorph":True,"needNewA":True,"needNewB":True,"newBisHot":False}
    need_new_a = not a["rec"] or a["period"]==1
    need_new_b = not b["rec"] or b["period"]==1
    new_a = a["rec"]; new_b = b["rec"]; new_b_is_hot = b.get("isHot",False)
    if need_new_a:
    ...
    if need_new_b:
        ...
        if max_cnt>=3:
            ...
            if hot_miss<2 and hot_combo!=new_a:
                ...
                new_b_is_hot = True
        new_b = b_candidate
    return {"a":new_a,"b":new_b,"aPeriod":a["period"],"bPeriod":b["period"],"cnt":cnt,"isMorph":False,"needNewA":need_new_a,"needNewB":need_new_b,"newBisHot":new_b_is_hot}
...
async def check_and_push(bot: Bot):
    ...
    # ---- 自动补全历史 ----
    if len(api_data)>=5 and latest_period != last_period:
        win_for_fill = api_data[1:MAX_WINDOW]
        for item in api_data:
            ...
            rec = get_recommendation(win_for_fill, temp_state)
            hit_a = (item["combo"]==rec["a"])
            hit_b = (item["combo"]==rec["b"])
            record = {
                "period": item["period"], "actual": item["combo"],
                "nums": item["nums"], "sum": item["sum"],
...
async def cmd_subscribe(update, context):
    ...
    else: subscribers.add(chat_id); save_all_state(); await update.message.reply_text("✅ 订阅成功！")
async def cmd_unsubscribe(update, context):
    chat_id = update.effective_chat.id
    if chat_id in subscribers: subscribers.discard(chat_id); save_all_state(); await update.message.reply_text("❌ 已取消订阅")
