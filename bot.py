...
def get_recommendation(win, state):
    ...
    if morph["triggered"]:
        ...
        return {"a":a_rec,"b":b_rec,"aPeriod":1,"bPeriod":1,"morph":morph,"cnt":cnt,"isMorph":True,
                "needNewA":True,"needNewB":True,"newBisHot":False}
    ...
    if need_new_b:
        ...
        if max_cnt >=3:
            ...
            if hot_miss <2 and hot_combo != new_a:
                ...
                new_b_is_hot = True
        new_b = b_candidate
    return {"a":new_a,"b":new_b,"aPeriod":a["period"],"bPeriod":b["period"],"morph":morph,"cnt":cnt,
            "isMorph":False,"needNewA":need_new_a,"needNewB":need_new_b,"newBisHot":new_b_is_hot}

def update_state(latest_result, win, state):
    a = state.get("a",{"period":1,"rec":""})
    b = state.get("b",{"period":1,"rec":"","isHot":False,"hotCombo":""})
    if latest_result:
...
async def check_and_push(bot: Bot):
    ...
    diag_line = f"📈 已记录 {len(history)} 期对错数据\n"
