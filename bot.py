...
def get_recommendation(win, state):
    ...
    if morph["triggered"]:
        m = {"连续出单": ("大单", "小单"), "连续出双": ("大双", "小双"), "大双小单交替": ("大双", "小单"), "大单小双交替": ("大单", "小双"),
...
# 全局状态
yinyu_state = load_json("state.json", {"a": {"period": 1, "rec": ""}, "b": {"period": 1, "rec": ""}})
...
async def check_and_push(bot: Bot):
    ...
    # 记录本期对错
    if prev_state.get("a", {}).get("rec") and prev_state.get("b", {}).get("rec"):
