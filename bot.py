#!/usr/bin/env python3
"""
游刃有余双冷方案 · 自动推送机器人 (稳定修复版)
- 数据持久化到 /data (需配合 Railway Volume)
- 启动时自动补全近期历史
- 滚动显示最近10个三期计划
- 单期命中率 = 历史所有期数中 A或B 至少一条命中的比例
- 延迟发布通过环境变量 DELAY_SECONDS 设置（秒）
"""
...
DATA_DIR = Path("/data")                     # 必须使用 /data 配合 Railway Volume
    ...
    if need_new_b:
        ...
        if max_cnt >= 3:
            ...
            if hot_miss < 2 and hot_combo != new_a:
                ...
                new_b_is_hot = True
        new_b = b_candidate
    a_logic = f"最冷({cnt[new_a]}次)"
    b_logic = f"🔥最热({cnt[new_b]}次)" if new_b_is_hot else f"次冷({cnt[new_b]}次)"
    return {"a": new_a, "b": new_b, "aPeriod": a["period"], "bPeriod": b["period"], "aLogic": a_logic, "bLogic": b_logic, "morph": morph, "cnt": cnt, "isMorph": False, "needNewA": need_new_a, "needNewB": need_new_b, "newBisHot": new_b_is_hot}


def update_state(latest_result: str | None, win: list, state: dict) -> dict:
    a = state["a"]
    b = state["b"]
    if latest_result:
...
def apply_recommendation(result: dict, state: dict) -> dict:
    if result["isMorph"]:
        state["a"]["rec"] = result["a"]
        state["a"]["period"] = 1
        state["b"]["rec"] = result["b"]
        state["b"]["period"] = 1
        state["b"]["isHot"] = False
        state["b"]["hotCombo"] = ""
    else:
...
def fill_history_gaps():
    ...
    if len(api_data) < 5:
        return
    all_periods = list(reversed(api_data))  # 从最新到最旧
    for i in range(len(all_periods) - 4):
        ...
        if any(h["period"] == target_period for h in history):
            continue
        # 模拟当时的推荐
        future = all_periods[i+1:]  # 比 target 更新的期次
        if len(future) < 4:
            continue
...
def calc_plan_stats(hist):
    """滚动三期计划统计，返回 (最近10个计划列表, 总期数, 命中期数)"""
    if len(hist) < 3:
        return [], 0, 0
    ...
    while i < len(sorted_hist):
        ...
        if hit:
            ...
            plans.append({"range": f"{start}～{end_period}", "success": True})
            i = end_idx + 1
        else:
            ...
            plans.append({"range": f"{start}～{end_period}", "success": False})
            i += 3
...
async def check_and_push(bot: Bot):
    ...
    # 7. 处理B线冲突
    display_b = result["b"]
    display_b_logic = result["bLogic"]
    if result["a"] == result["b"]:
        cnt = result["cnt"]
        remaining = [k for k in cnt if k != result["a"]]
        remaining.sort(key=lambda k: cnt[k])
        if remaining:
            display_b = remaining[0]
            display_b_logic = f"次冷({cnt[display_b]}次)（{result['b']}冲突顺延）"
    ...
    message = (
        ...
        f"━━━━━━━━━━━━━━━━\n"
        f"🎯 <b>本期推荐</b>\n"
        f"🔵 A线：<b>{result['a']}</b>（{result['aLogic']}）第{result['aPeriod']}期\n"
        f"🔴 B线：<b>{display_b}</b>（{display_b_logic}）第{result['bPeriod']}期\n"
        f"━━━━━━━━━━━━━━━━\n"
...
def main():
    ...
    if os.environ.get("WEBHOOK_URL"):
        app.run_webhook(listen="0.0.0.0", port=int(os.environ.get("PORT", "8080")), url_path=BOT_TOKEN, webhook_url=f"{os.environ['WEBHOOK_URL']}/{BOT_TOKEN}")
    else:
